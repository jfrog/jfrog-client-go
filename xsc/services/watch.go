...
func (xws *WatchService) GetResourceWatches(gitRepo, project string) (watches *api.ResourcesWatchesBody, err error) {
	...
	log.Info(fmt.Sprintf("Found %d active watches in the follwoing resources - %s", len(watches.GitRepositoryWatches)+len(watches.ProjectWatches), getResourcesString(gitRepo, project)))
	return
}
