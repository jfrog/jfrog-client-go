...
func (vs *AnalyticsEventService) AddGeneralEvent(event XscAnalyticsGeneralEventDeprecated) (string, error) {
	var requestContent []byte
	xrayVersion, err := vs.XrayDetails.GetVersion()
	if err != nil {
		return "", err
	}
	if err := utils.ValidateMinimumVersion(utils.Xray, xrayVersion, services.MinXrayVersionGitRepoKey); err == nil {
		// use new event struct
		updatedEvent := convertToNewEventStruct(event)
		requestContent, err = json.Marshal(updatedEvent)
		if err != nil {
...
func convertToNewEventStruct(event XscAnalyticsGeneralEventDeprecated) XscAnalyticsGeneralEvent {
	var updatedGitInfo XscGitInfoContext
	if event.GitInfo != nil {
		updatedGitInfo = XscGitInfoContext{
			Source: CommitContext{
				GitRepoHttpsCloneUrl: event.GitInfo.GitRepoUrl,
				GitRepoName:          event.GitInfo.GitRepoName,
				GitProject:           event.GitInfo.GitProject,
				BranchName:           event.GitInfo.BranchName,
				LastCommitHash:       event.GitInfo.CommitHash,
				LastCommitMessage:    event.GitInfo.CommitMessage,
				LastCommitAuthor:     event.GitInfo.CommitAuthor,
			},
			PullRequest:  nil,
	...
	return XscAnalyticsGeneralEvent{
