...
// You may implement the log.ProgressBar interface, or pass nil to run without progress bar.
func (jc *HttpClient) UploadFile(localPath, url, logMsgPrefix string, httpClientsDetails httputils.HttpClientDetails,
	retries int, progressBar log.ProgressBar) (resp *http.Response, body []byte, err error) {
	retryExecutor := utils.RetryExecutor{
		...
		ExecutionHandler: func() (bool, error) {
			resp, body, err = jc.doUploadFile(localPath, url, httpClientsDetails, progressBar)
			if err != nil {
				return true, err
			}
...
func (jc *HttpClient) doUploadFile(localPath, url string, httpClientsDetails httputils.HttpClientDetails, progressBar log.ProgressBar) (*http.Response, []byte, error) {
	var file *os.File
	var err error
	if localPath != "" {
	...
	if file != nil && progressBar != nil {
		barId := progressBar.NewBar(size, "Uploading", localPath)
		reader = progressBar.ReadWithProgress(barId, reqContent)
		defer progressBar.Abort(barId)
	} else {
		reader = reqContent
	}
...
func (jc *HttpClient) DownloadFileWithProgress(downloadFileDetails *DownloadFileDetails, logMsgPrefix string,
	httpClientsDetails httputils.HttpClientDetails, retries int, isExplode bool, progressBar log.ProgressBar) (*http.Response, error) {
	resp, _, err := jc.downloadFile(downloadFileDetails, logMsgPrefix, true, httpClientsDetails, retries, isExplode, progressBar)
	return resp, err
}
...
func (jc *HttpClient) downloadFile(downloadFileDetails *DownloadFileDetails, logMsgPrefix string, followRedirect bool,
	httpClientsDetails httputils.HttpClientDetails, retries int, isExplode bool, progressBar log.ProgressBar) (resp *http.Response, redirectUrl string, err error) {
	retryExecutor := utils.RetryExecutor{
		MaxRetries:      retries,
		RetriesInterval: 0,
		...
		LogMsgPrefix:    logMsgPrefix,
		ExecutionHandler: func() (bool, error) {
			resp, redirectUrl, err = jc.doDownloadFile(downloadFileDetails, logMsgPrefix, followRedirect, httpClientsDetails, isExplode, progressBar)
...
func (jc *HttpClient) doDownloadFile(downloadFileDetails *DownloadFileDetails, logMsgPrefix string, followRedirect bool,
	httpClientsDetails httputils.HttpClientDetails, isExplode bool, progressBar log.ProgressBar) (resp *http.Response, redirectUrl string, err error) {
	...
	err = saveToFile(downloadFileDetails, resp, progressBar)
	if err != nil {
		return
	}
...
func saveToFile(downloadFileDetails *DownloadFileDetails, resp *http.Response, progressBar log.ProgressBar) error {
	...
	var reader io.Reader
	if progressBar != nil {
		barId := progressBar.NewBar(resp.ContentLength, "Downloading", downloadFileDetails.RelativePath)
		reader = progressBar.ReadWithProgress(barId, resp.Body)
		defer progressBar.Abort(barId)
	} else {
		reader = resp.Body
	}
...
func (jc *HttpClient) DownloadFileConcurrently(flags ConcurrentDownloadFlags, logMsgPrefix string,
	httpClientsDetails httputils.HttpClientDetails, progressBar log.ProgressBar) (*http.Response, error) {
	...
	var downloadBarId int
	if progressBar != nil {
		downloadBarId = progressBar.NewBar(flags.FileSize, "Downloading", flags.RelativePath)
		mergingBarId := progressBar.NewBarReplacement(downloadBarId, "  Merging  ", flags.RelativePath)
		// Aborting order matters. mergingBar depends on the existence of downloadingBar
		defer progressBar.Abort(downloadBarId)
		defer progressBar.Abort(mergingBarId)
	}

	resp, err := jc.downloadChunksConcurrently(chunksPaths, flags, logMsgPrefix, tempDirPath, httpClientsDetails, progressBar, downloadBarId)
	if err != nil {
		return nil, err
	}
...
func (jc *HttpClient) downloadChunksConcurrently(chunksPaths []string, flags ConcurrentDownloadFlags, logMsgPrefix,
	chunksDownloadPath string, httpClientsDetails httputils.HttpClientDetails, progressBar log.ProgressBar, barId int) (*http.Response, error) {
	...
	for i := 0; i < flags.SplitCount; i++ {
		...
		go func(start, end int64, i int) {
			chunksPaths[i], respList[i], errorsList[i] = jc.downloadFileRange(flags, start, end, i, logMsgPrefix, chunksDownloadPath, *requestClientDetails, flags.Retries, progressBar, barId)
...
func (jc *HttpClient) downloadFileRange(flags ConcurrentDownloadFlags, start, end int64, currentSplit int, logMsgPrefix, chunkDownloadPath string,
	httpClientsDetails httputils.HttpClientDetails, retries int, progressBar log.ProgressBar, barId int) (fileName string, resp *http.Response, err error) {
	retryExecutor := utils.RetryExecutor{
		MaxRetries:      retries,
		RetriesInterval: 0,
		...
		ExecutionHandler: func() (bool, error) {
			fileName, resp, err = jc.doDownloadFileRange(flags, start, end, currentSplit, logMsgPrefix, chunkDownloadPath, httpClientsDetails, progressBar, barId)
			if err != nil {
				return true, err
			}
			// Response must not be nil
...
func (jc *HttpClient) doDownloadFileRange(flags ConcurrentDownloadFlags, start, end int64, currentSplit int, logMsgPrefix, chunkDownloadPath string,
	httpClientsDetails httputils.HttpClientDetails, progressBar log.ProgressBar, barId int) (fileName string, resp *http.Response, err error) {
