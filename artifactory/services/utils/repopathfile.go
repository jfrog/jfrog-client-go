...
func createRepoPathFileTriples(pattern string, recursive bool) []RepoPathFile {
	...
	if firstSlashIndex < 0 {
		return createPathFilePairs("*", "*", recursive)
	}
...
func createPathFilePairs(repo, pattern string, recursive bool) []RepoPathFile {
	...
	path, name, triples := handleNonRecursiveTriples(repo, pattern)
	if !recursive {
		return triples
	}
	if name == "*" {
...
func handleNonRecursiveTriples(repo, pattern string) (string, string, []RepoPathFile) {
