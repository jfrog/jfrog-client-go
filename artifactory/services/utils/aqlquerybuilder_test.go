...
var buildAqlSearchQueryDataProvider = []struct {
	...
	{"repo-w*ldcard", true,
		`{"$or":[{"$and":[{"repo":{"$match":"repo-w*"},"path":{"$match":"*ldcard"},"name":{"$match":"*"}}]},{"$and":[{"repo":{"$match":"repo-w*"},"path":{"$match":"*ldcard/*"},"name":{"$match":"*"}}]},{"$and":[{"repo":{"$match":"repo-w*ldcard"},"path":{"$match":"*"},"name":{"$match":"*"}}]}]}`},
	...
	{"*repo-local", false,
		`{"$or":[{"$and":[{"repo":{"$match":"*"},"path":{"$match":"*repo-local"},"name":{"$match":"*"}}]},{"$and":[{"repo":{"$match":"*repo-local"},"path":".","name":{"$match":"*"}}]}]}`},
...
func TestBuildAqlSearchQuery(t *testing.T) {
	for _, sample := range buildAqlSearchQueryDataProvider {
		t.Run(sample.pattern+"_recursive_"+strconv.FormatBool(sample.recursive), func(t *testing.T) {
			params := ArtifactoryCommonParams{Pattern: sample.pattern, Target: "", Props: "", Build: "", Recursive: sample.recursive, Regexp: false, IncludeDirs: false}
