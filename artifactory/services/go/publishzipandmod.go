...
func (pwa *publishZipAndModApi) PublishPackage(params GoParams, client *rthttpclient.ArtifactoryHttpClient, ArtDetails auth.ArtifactoryDetails) error {
	...
	resp, _, err := client.UploadFile(params.GetZipPath(), zipUrl, zipUrl, &clientDetails, GoUploadRetries)
	if err != nil {
		return err
	}
	...
	resp, _, err = client.UploadFile(params.GetModPath(), url, url, &clientDetails, GoUploadRetries)
	if err != nil {
		return err
	}
