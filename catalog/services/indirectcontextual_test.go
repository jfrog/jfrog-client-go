...
func newTestIndirectContextualService(t *testing.T, serverUrl string) *IndirectContextualService {
	details := &testCatalogDetails{}
	details.SetUrl(serverUrl + "/")
	client, err := jfroghttpclient.JfrogClientBuilder().Build()
	require.NoError(t, err)
	service := NewIndirectContextualService(client)
	service.CatalogDetails = details
	return service
}

func TestGetContextualPaths_SendsExpectedRequestAndParsesResponse(t *testing.T) {
	var gotBody indirectContextualRequest
	var gotQuery string
