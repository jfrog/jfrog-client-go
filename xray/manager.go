...
func (sm *XrayServicesManager) ImportGraph(params services.XrayGraphImportParams, rootPath string) (scanId string, err error) {
	enrichService := services.NewEnrichService(sm.client)
	enrichService.XrayDetails = sm.config.GetServiceDetails()
	return enrichService.ImportGraph(params, rootPath)
}
